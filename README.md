...
The current implementation is a deterministic local prototype by default. Search results are mock data, LLM calls are optional, and evaluation uses rule metrics.

## Current Status

The repository is currently at Phase 9: LLM Client and Prompt System.
...
This phase does not include online search, concurrent execution, vector memory, LLM-as-judge, Bootstrap CI, Cohen's d, or external benchmark downloads.
...
By default the project uses `MockLLMClient`. To try an OpenAI-compatible endpoint, set environment variables:
