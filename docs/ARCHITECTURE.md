...
The current system uses mock search/fetch and deterministic local logic by default. Optional LLM calls and optional standard-library web search/fetch are available when configured. Citation grounding is local and rule-based; the system does not implement production-grade webpage parsing, semantic evidence verification, vector memory, concurrent DAG execution, LLM-as-Judge, or external benchmark downloads.
